...
namespace CVDimage
{
	...
	void pred_2dbayer_diff(const byte* in, int width, byte* buffer1, byte* buffer2, byte* out, bool greenfirst)
	{
		...
		if(greenfirst)
		{
			int pred = (pred_coeff_green[0] * in[i - 2] + pred_coeff_green[1] * buffer2[i - 1] + pred_coeff_green[2] * buffer1[i]) / 8;
			out[i] = static_cast<byte>((256 + in[i] - pred) & 255);
			buffer1[i - 2] = in[i - 2];
			i++;
		}
		...
		for(; i < width; i++)
		{
			int pred = (pred_coeff_bluered[0] * in[i - 2] + pred_coeff_bluered[1] * buffer1[i - 2] + pred_coeff_bluered[2] * buffer1[i]) / 8;
			out[i] = static_cast<byte>((256 + in[i] - pred) & 255);
			buffer1[i - 2] = in[i - 2];
			i++;
			if(!(i < width))
				break;
			pred = (pred_coeff_green[0] * in[i - 2] + pred_coeff_green[1] * buffer2[i - 1] + pred_coeff_green[2] * buffer1[i]) / 8;
			out[i] = static_cast<byte>((256 + in[i] - pred) & 255);
			buffer1[i - 2] = in[i - 2];
		}
	...
	void pred_2dbayer_undiff(const byte* in, int width, byte* buffer1, byte* buffer2, byte* out, bool greenfirst)
	{
		...
		if(greenfirst)
		{
			int pred = (pred_coeff_green[0] * out[i - 2] + pred_coeff_green[1] * buffer2[i - 1] + pred_coeff_green[2] * buffer1[i]) / 8;
			out[i] = static_cast<byte>((in[i] + pred) & 255);
			buffer1[i - 2] = out[i - 2];
			i++;
		}
		...
		for(; i < width; i++)
		{
			int pred = (pred_coeff_bluered[0] * out[i - 2] + pred_coeff_bluered[1] * buffer1[i - 2] + pred_coeff_bluered[2] * buffer1[i]) / 8;
			out[i] = static_cast<byte>((in[i] + pred) & 255);
			buffer1[i - 2] = out[i - 2];
			i++;
			if(!(i < width))
				break;
			pred = (pred_coeff_green[0] * out[i - 2] + pred_coeff_green[1] * buffer2[i - 1] + pred_coeff_green[2] * buffer1[i]) / 8;
			out[i] = static_cast<byte>((in[i] + pred) & 255);
			buffer1[i - 2] = out[i - 2];
		}
	...
	template <class P>
	void huff_decompress(const vector<P>& b, const array<size_t, 256>& h, Image<byte>& ret)
	{
		vector<Huff*> terminals;
	...
	void ReadPimpl::read_header(istream& in)
	{
		...
		if(tmp != "CVD")
			throw CVD::Exceptions::Image_IO::MalformedImage(string("Error in CVD image: incorrect header ID"));
		...
		else if((type == "bayer_grbg") || (type == "bayer_gbrg"))
		{
			...
			throw CVD::Exceptions::Image_IO::MalformedImage(string("Error in CVD image: unknown data type"));
	}
	...
	{
		swap(buffer, buffer2);
		if(pred_mode == PRED_2D_BAYER_1)
			pred_mode = PRED_2D_BAYER_2;
		else if(pred_mode == PRED_2D_BAYER_2)
			pred_mode = PRED_2D_BAYER_1;
	}
	...
	template <class C>
	void WritePimpl::write_raw_pixel_line(const C* d)
	{
	...
	void WritePimpl::write_header(std::ostream& os)
	{
		os << "CVD\n"
		   << type << "\n"
		   << xs << " " << ys;
		if(pred_mode == PRED_2D_BAYER_2)
			pred_mode = PRED_2D_BAYER_1;
		if(pred_mode != PRED_HORIZONTAL)
			os << " pred=" << (int)pred_mode;
		os << "\n";
	}
