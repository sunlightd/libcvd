...
#undef GET
	...
	template <class C>
	void get_num(istream& i, Rgb<C>* c)
	{
		get_num(i, &(c->red));
		get_num(i, &(c->green));
		get_num(i, &(c->blue));
...
#define GEN1(X) \
	void Reader::get_raw_pixel_line(X* d) { p->get_raw_pixel_lines(d, 1); }
#define GEN2(X) \
	GEN1(X)     \
	GEN1(Rgb<X>)
	GEN1(bool)
...
#undef GEN2
	...
	template <class C>
	void write_text(const C* data, size_t n, ostream& o)
	{
	...
	template <class C>
	void write_text(const Rgb<C>* data, size_t n, ostream& o)
	{
		for(size_t i = 0; i < n; i++)
...
#define GEN1(X) \
	void Writer::write_raw_pixel_line(const X* d) { p->write_raw_pixel_line(d); }
#define GEN2(X) \
	GEN1(X)     \
	GEN1(Rgb<X>)
	//GEN1(bool)
